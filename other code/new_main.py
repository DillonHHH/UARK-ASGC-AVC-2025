...
ANGLE_CENTERING_TOLERANCE_DEGREES = 2
ORBIT_DISTANCE_METERS = 1
# BUCKET_START_ACTION_DISTANCE_METERS: float = (
#     2  # once a bucket is this close the action for the given bucket will start
# )

LIDAR_VALUE_MULTIPLIER = 1000  # mm to m

MIN_LIDAR_SCAN_BUFFFER_LENGTH = (
    360  # at least this many scan must be made before processing any scans
)
...
def get_full_scan_buffer():
    ...
    while len(scan_buffer) < MIN_LIDAR_SCAN_BUFFFER_LENGTH:
...
def orbit(next_obstacle: TargetObstacle, clockwise: bool):
    ...
    #     set_wheel_angle(135)
    """    
    #LIDAR_ACTIVATION_RANGE = (80, 100) if clockwise else (260, 280)
    logger.debug("About to enter angle checking loop")
    #while True:
    logger.debug("getting full scan buffer")
    scan_buffer = get_full_scan_buffer()
    logger.debug("got full scan buffer")
    closest_scan: DAQ = DAQ(math.inf, 0, 0)
    for scan in scan_buffer:
        if scan.distance < closest_scan.distance:
            closest_scan = scan
    logger.debug("found closest lidar scan, checking angle")
    #if (
    #    closest_scan.angle_degrees > LIDAR_ACTIVATION_RANGE[0]
    #    and closest_scan.angle_degrees < LIDAR_ACTIVATION_RANGE[1]
    #):
    logger.debug("found lidar scan within angle")
    """
    ...
    while True:
        ...
        bucket_x, bucket_y = (
            closest_lidar_scan.to_relative_location()
        )  # Note output will be sin then cos

        # if(next_obstacle.type == OBSTAClE_TYPES.BLUE):
        #    bucket_x = bucket_x - 1
        # elif(next_obstacle.type == OBSTAClE_TYPES.YELLOW):
        #    bucket_x = bucket_x + 1

        logger.debug(f"Closest lidar scan location: {(bucket_x, bucket_y)}")

        # The following 3 floats are in degrees
        theta_offset: float = closest_lidar_scan.angle_degrees
        theta_found: float = math.atan2(bucket_x, bucket_y)
        theta_servo_offset: float = 90

        theta_drive: float = theta_offset + theta_found + theta_servo_offset
        ...
        if theta_drive < 45:
            theta_drive = 45
        elif theta_drive > 135:
            theta_drive = 135

        set_wheel_angle(int(theta_drive))
        carControls.setRot(current_wheel_angle)
...
def _navigate_to_location(location: tuple[float, float]):
    ...
    if target_location_angle_from_center_degrees < -ANGLE_CENTERING_TOLERANCE_DEGREES:
        set_wheel_angle(STRAIGHT_WHEEL_ANGLE_DEGREES - 15)
...
def navigate_to_yellow_or_blue_bucket(target_obstacle: TargetObstacle) -> None:
    ...
    OFFSET_FROM_TARGET_METERS: tuple[float, float] = (
        (-1, 0) if target_obstacle.type == OBSTAClE_TYPES.BLUE else (1, 2)
    )

    FAILED_DETECTION_TRESHOLD = 5  # After failing to find the target for this many frames, car will begin roaming

    CLOSEST_PREVIOUS_DISTANCE_BEFORE_ORBIT = 2.5
    ...
    while True:
        ...
        if frame:
            ...
            if target_object:
                ...
                if closest_previous_distance < CLOSEST_PREVIOUS_DISTANCE_BEFORE_ORBIT:

                    return
                elif consecutive_failed_detections > FAILED_DETECTION_TRESHOLD:
                    logger.warning(
...
def get_target_object(
    ...
    for detected_object in detected_objects:
        ...
        if target_obstacle.type.value.lower() in detected_object.classification.lower():
            detected_object_location: Union[tuple[float, float], None] = (
                detector.get_cartesian_location_from_pixel(
                    frame, detected_object.box.get_center()
                )
            )
